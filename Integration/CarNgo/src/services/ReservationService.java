...
public class ReservationService implements IReservationService {
    ...
    public boolean ajouterReservation(Reservation res) {
        ...
        try {
            ...
            ps.setDate(1, res.getDatedebut());
            ps.setDate(2, res.getDatefin());
    ...
    public boolean modifierReservation(int idreservation, Reservation res) {
        ...
        try {
            ...
            ste.setDate(2, res.getDatedebut());
            ste.setDate(3, res.getDatefin());
            ste.setString(4, EtatReservation.en_cours.toString());
...
//  
    ...
    public ObservableList <Reservation> getReservationbyIdUserNDStatus(int iduser) {
        ObservableList<Reservation> list = FXCollections.observableArrayList();
        ...
        try {
            String req = "SELECT r.*, u.cin, v.matricule, i.pointdepart, i.pointarrivee "
                    ...
                    + "JOIN vehicule v ON v.idvehicule = r.idvehicule "
                    + "JOIN itineraire i ON i.iditineraire = r.iditineraire "
                    + "WHERE r.iduser=? AND r.status='en_cours' ";
            PreparedStatement ps = myConnex.prepareStatement(req);
           ps.setInt(1, iduser);
            ResultSet resu = ps.executeQuery();
            while (resu.next()) {
 ...
 public int getIdReservationbyIdUserNDStatus(int iduser) {   
     Reservation res = new Reservation();
int id =-1;
     System.out.println("iduser:::"+iduser);
        try {
    ...
    public double calculerTarification(Reservation reservation, itineraire itineraire) {
        float tarifDeBase = 5;
        double tarifMinute = 0.1;
        double tarifKilometrique = 0.5;
        float kilometrage = itineraire.getKilometrage();
        Reservation res = new Reservation();
        Date dateDebut = res.getDatedebut();
        LocalDateTime localDateTimeDebut = dateDebut.toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime();
        Date dateFin = res.getDatefin();
        LocalDateTime localDateTimeFin = dateFin.toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime();
        Duration duree = Duration.between(localDateTimeDebut, localDateTimeFin);
        long dureeEnMinutes = ChronoUnit.MINUTES.between(localDateTimeDebut, localDateTimeFin);

        double coutTotal = tarifDeBase
                + (dureeEnMinutes * tarifMinute) + (kilometrage * tarifKilometrique);
        return coutTotal;
    }
