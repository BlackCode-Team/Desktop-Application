...
public class AjouterVehiculeController implements Initializable {
    ...
    @FXML
    private ChoiceBox<String> txtmodele;
    @FXML
    ...
    void add(ActionEvent event) throws IOException{
        ...
        v.setModele(txtmodele.getValue());
    ...
    public boolean isValid() {
        if (txtmatricule.getText().isEmpty()||txtprix.getText().isEmpty()||txtpuissance.getText().isEmpty()||txtmodele.getValue() == null || choiceboxtype.getValue() == null){
        ...
        if (txtmodele.getValue() == null){
            Alert al = new Alert(Alert.AlertType.ERROR);
            al.setTitle("champs vide");
            al.setHeaderText("Veuillez choisir le modele de vehicule ");
            al.showAndWait();
            return false;
        }
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        choiceboxtype.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> {
            // Récupération du type de véhicule sélectionné
            TypeVehicule selectedType = (TypeVehicule) newValue;

            // Remplissage de la ChoiceBox txtmodele en fonction du type de véhicule sélectionné
            if (selectedType == TypeVehicule.voiture) {
                txtmodele.getItems().setAll("Dacia Spring" ,"Kia e-Niro","Mercedes eqc","Mg 4" ,"Peugeot e208 ", "Tesla Model 3");
            } else if (selectedType == TypeVehicule.trottinette) {
                txtmodele.getItems().setAll("Hiley Tiger 10 pro","Pure air pro","Wispeed T850","Xiaomi 1s");
            } else {
                // Par défaut, on vide la ChoiceBox txtmodele si aucun choix n'a été sélectionné
                txtmodele.getItems().clear();
            }
        });
