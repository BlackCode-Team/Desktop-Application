...
//            currentidit =id;
 ...
 List<itineraire> itineraires = new ArrayList<>();
        try {
            String sql = "select pointDepart,pointarrivee,dureeestimee,iditineraire from itineraire c where iduser=?";
           PreparedStatement ste = cnx.prepareStatement(sql);
            ste.setInt(4, id);
            ste.executeUpdate();
            ...
            while (s.next()) {
                ...
                itineraires.add(i);
                int idit= s.getInt("iditineraire");
                currentidit =idit;
    ...
    public int selectionParId(int id) {
        try {
            String sql = "SELECT iditineraire FROM itineraire WHERE iduser=?";
            PreparedStatement ste = cnx.prepareStatement(sql);
            ste.setInt(1, id);
            ResultSet s = ste.executeQuery();
            while (s.next()) {
                currentidit = s.getInt("iditineraire");
            }
        } catch (SQLException ex) {
            System.out.println(ex.getMessage());
        }
        return currentidit; // returning an array of exact size
    }
